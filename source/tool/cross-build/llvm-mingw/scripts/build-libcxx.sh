...
build_all() {
    ...
    for arch in $ARCHS; do
	    ...
	    ${CXXABI_BASIC_FLAGS} \
            ...
            -DLIBCXXABI_LIBCXX_INCLUDE_PATHS=../../libcxx/include \
            -DLIBCXXABI_LIBCXX_INCLUDES=../../libcxx/include \
	    -DLIBCXXABI_USE_LLVM_UNWINDER=ON \
	    -DLIBCXXABI_LIBUNWIND_PATH=$PREFIX/$arch-w64-mingw32/lib \
            ..
#        make -j$CORES -l ${WORKLOADS}
#        make install
#        if [ "$type" = "shared" ]; then
...
#	fi    
    ...
    for arch in $ARCHS; do
	    ...
	    ${CXX_BASIC_FLAGS} \
            ...
            -DLIBCXX_TARGET_TRIPLE=$arch-w64-mingw32 \
            -DLIBCXX_CXX_ABI_INCLUDE_PATHS=../../libcxxabi/include \
            -DLIBCXX_CXX_ABI_LIBRARY_PATH=../../libcxxabi/build-$arch-$type/lib \
            -DCMAKE_CXX_FLAGS="$LIBCXX_VISIBILITY_FLAGS" \
	    -DLIBCXX_LIBUNWIND_PATH=$PREFIX/$arch-w64-mingw32/lib \
            -DCMAKE_SHARED_LINKER_FLAGS="-lunwind" \
            ..
	    
	    make generate-cxx-headers
#        make -j$CORES -l ${WORKLOADS}
#        make install
#        if [ "$type" = "shared" ]; then
#            mkdir -p $PREFIX/$arch-w64-mingw32/bin
...
#	fi    
    ...
    for arch in $ARCHS; do
	    ...
	    ${CXXABI_BASIC_FLAGS} \
            ...
            -DLIBCXXABI_LIBCXX_INCLUDE_PATHS=../../libcxx/build-$arch-$type/include/c++/v1 \
            -DLIBCXXABI_LIBDIR_SUFFIX="" \
            ..
    ...
    for arch in $ARCHS; do
	    ...
	    ${CXX_BASIC_FLAGS} \
            ...
            -DLIBCXX_TARGET_TRIPLE=$arch-w64-mingw32 \
            -DLIBCXX_CXX_ABI_INCLUDE_PATHS=../../libcxxabi/include \
            -DLIBCXX_CXX_ABI_LIBRARY_PATH=$PREFIX/$arch-w64-mingw32/lib \
            -DCMAKE_CXX_FLAGS="$LIBCXX_VISIBILITY_FLAGS" \
	    -DLIBCXX_LIBUNWIND_PATH=$PREFIX/$arch-w64-mingw32/lib \
            -DCMAKE_SHARED_LINKER_FLAGS="-lunwind" \
            ..
