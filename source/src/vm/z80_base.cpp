...
Z80_INLINE uint8_t Z80_BASE::DEC(uint8_t value)
{
	uint8_t res = value - 1;
	F = (F & CF) | SZHV_dec[res];
	return res;
...
void Z80_BASE::OP_CB(uint8_t code)
{
	icount -= cc_cb[code];
...
void Z80_BASE::OP_XY(uint8_t code)
{
	icount -= cc_xycb[code];
...
void Z80_BASE::OP_DD(uint8_t code)
{
	icount -= cc_xy[code];
...
void Z80_BASE::OP_FD(uint8_t code)
{
	icount -= cc_xy[code];
...
void Z80_BASE::OP_ED(uint8_t code)
{
	icount -= cc_ed[code];
...
bool Z80_BASE::write_debug_reg(const _TCHAR *reg, uint32_t data)
{
