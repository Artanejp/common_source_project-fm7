...
{
	d_beep->write_signal(SIG_BEEP_MUTE, ~data, 0x01);
	if((data & 0x40) == 0) {
		d_beep->write_signal(SIG_BEEP_ON, data, 0x80);
	} else {
		set_beep_oneshot();
	}
}
...
void FM7_MAINIO::set_beep_oneshot(void) // SUB:D4xx
{
	...
	d_beep->write_signal(SIG_BEEP_ON, 0xff, 0x80);
	
	//if(event_beep_oneshot >= 0) cancel_event(this, event_beep_oneshot); // OK?
	if(event_beep_oneshot < 0) {
