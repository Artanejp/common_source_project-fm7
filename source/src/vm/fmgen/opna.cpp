...
void OPN2Base::Reset()
{
	...
	SetReg(0x27, 0x30);
	prescale = 10;
	SetPrescaler(1); // Maybe, default prescaler value is 3.From YM2612 of Tsugaru emulator.
	OPNBase::Reset();
	SetReg(0x27, 0x30);
	SetReg(0x26, 0x00);
	SetReg(0x25, 0x00);
...
{
	static const uint8 table_prescale[4] = {6, 3, 2};
	static const uint8 table2[8] = { 108,  77,  71,  67,  62,  44,  8,  5 };
	if(p == prescale) return;
	if(p >= 3) return;

	prescale = p;
	uint fmclock = clock / table_prescale[p] / 12;
	rate = psgrate;
...
//	assert(fmclock < (0x80000000 >> FM_RATIOBITS));
	...
	SetTimerPrescaler(table_prescale[p] * 12 * 2);
	//MakeTimeTable(ratio);
	chip.SetRatio(ratio);
