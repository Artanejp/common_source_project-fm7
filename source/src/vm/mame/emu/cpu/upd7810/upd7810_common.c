...
struct opcode_s {
	void (__FASTCALL *opfunc)(upd7810_state *cpustate);
	UINT8 oplen;
	UINT8 cycles;
	UINT8 cycles_skip;
	UINT8 mask_l0_l1;
...
static void __FASTCALL WP(upd7810_state *cpustate, offs_t port, UINT8 data)
{
	switch (port)
	{
	case UPD7810_PORTA:
...
static void __FASTCALL upd7810_write_EOM(upd7810_state *cpustate)
{
...
static void __FASTCALL upd7810_write_TXB(upd7810_state *cpustate)
{
	cpustate->txbuf = 1;
}
...
static void __FASTCALL upd7810_sio_output(upd7810_state *cpustate)
{
	/* shift out more bits? */
	if (cpustate->txcnt > 0)
	{
...
static void __FASTCALL upd7810_sio_input(upd7810_state *cpustate)
{
	/* sample next bit? */
	if (cpustate->rxcnt > 0)
	{
...
static void __FASTCALL upd7810_timers(upd7810_state *cpustate, int cycles)
{
	/**** TIMER 0 ****/
...
static void __FASTCALL upd7801_timers(upd7810_state *cpustate, int cycles)
{
	if ( cpustate->ovc0 )
	{
...
static void __FASTCALL upd78c05_timers(upd7810_state *cpustate, int cycles)
{
	if ( cpustate->ovc0 ) {
		cpustate->ovc0 -= cycles;
