...
void IO::initialize()
{
	...
	__IO_DEBUG_LOG_DEFAULT = __IO_DEBUG_LOG; // ToDo: Available to change by UI.
...
//#ifdef _IO_DEBUG_LOG
	...
	__UNLIKELY_IF(wr_table[laddr].is_flipflop) {
		rd_table[laddr].value = data & 0xff;
	} else if(is_dma) {
		wr_table[laddr].dev->write_dma_io8w(addr2, data & 0xff, &wait_tmp);
		__UNLIKELY_IF(!wr_table[laddr].wait_registered) *wait = wait_tmp;
	} else {
