...
void UPD7220::initialize()
{
	...
	if(osd->check_feature(_T("CHARS_PER_LINE"))) {
		_CHARS_PER_LINE = (int)(osd->get_feature_int_value(_T("CHARS_PER_LINE")));
	} else {
		_CHARS_PER_LINE = 0;
	}
...
{
	wrote_bytes = (params_count > 8) ? 8 : params_count;
...
{
	wrote_bytes = params_count;
	if(wrote_bytes > 3) wrote_bytes = 3;
	if(params_count > 0) {
		ead = params[0];
		if(params_count > 1) {
			ead |= params[1] << 8;
